...
const getIndexesFromEvent = (event: SortableEvent) => {
  if (
    Object.prototype.hasOwnProperty.call(event, "newDraggableIndex") &&
    Object.prototype.hasOwnProperty.call(event, "oldDraggableIndex")
  ) {
    return {
...
export class SortablejsDirective implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: { [prop in keyof SortablejsDirective]?: SimpleChange }) {
    ...
    if (optionsChange && !optionsChange.isFirstChange()) {
      ...
      (Object.keys(currentOptions) as (keyof Options)[]).forEach(
        (optionName) => {
  ...
  private get optionsWithoutEvents(): Options {
    return { ...(this.globalConfig || {}), ...(this.sortablejsOptions || {}) };
  }

  private proxyEvent(eventName: string, ...params: unknown[]) {
    this.zone.run(() => {
      // re-entering zone, see https://github.com/SortableJS/angular-sortablejs/issues/110#issuecomment-408874600
      const options = this.optionsWithoutEvents as Options &
        Record<string, unknown>;
      const handler = options[eventName];
  ...
  private clone<T>(item: T): T {
    ...
    return (this.sortablejsCloneFunction || ((subitem: T) => subitem))(item);
  }
